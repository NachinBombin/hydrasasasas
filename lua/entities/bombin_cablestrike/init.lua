...
function ENT:Initialize()
    ...
    -- Standalone: base is self until a parent sets it externally
    ...
    if IsValid(npcfix) then
        npcfix:SetPos( self:GetPos() + self:GetUp() * 20 )
...
function ENT:Think()
    -- Guard: ensure self.base is always valid
    ...
    if self.timestamp < CurTime() then
        ...
        if self.act == "idle" then
            ...
            for _,i in pairs(targets) do
                ...
                if (i:GetPos():Distance( self:GetPos() ) < distance
                    && self:Visible(i)
                    && i != self.Entity
                    && i != self.base
                    && !table.HasValue(self.ignoretable, iclass)) then
                    if i:IsNPC() then
