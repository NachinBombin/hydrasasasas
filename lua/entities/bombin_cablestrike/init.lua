...
function ENT:Initialize()
   ...
   -- Standalone: base is self until a parent entity sets it
   ...
   local npcfix = ents.Create("bombin_cablestrike_npcfix")
   if IsValid(npcfix) then
...
function ENT:Think()
    -- Always keep base valid; fall back to self if never set
...
function ENT:PhysicsSimulate(phys, dtime)
    ...
    -- Nil-safe base position
    local basepos = (IsValid(self.base) and self.base != self.Entity) and self.base:GetPos() or self:GetPos()
    ...
    self.ShadowParams.pos = basepos + Vector(0,0,300) + self.wanderoffset + prestrike

    if (self.act == "strike" && IsValid(self.target)) then
    ...
    if self.target == self.Entity or !IsValid(self.target) then
    ...
    if self.act == "hurt" then
        ...
        self.ShadowParams.pos = basepos + Vector(0,0,20)
    end
    ...
    self.ShadowParams.deltatime = dtime
