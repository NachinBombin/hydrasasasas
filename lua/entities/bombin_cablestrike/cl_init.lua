...
language.Add("sent_bombin_cablestrike", "Cable Strike")

local csr_res = CreateClientConVar( "cl_bombin_cablestrike_res", 15, true )
...
function ENT:Draw()
    local res = math.Clamp( csr_res:GetInt(), 5, 40 )
    local base = self:GetNetworkedEntity("base", self.Entity)
    if (!IsValid(base)) then return end
