...
function ENT:Initialize()
   ...
   self.Entity:SetModel( "models/improvedhydra/hydrahead_smacker.mdl" ) 
   ...
   light:SetModel( "models/improvedhydra/hydrahead_smacker.mdl" ) 
    light:SetMaterial("models/hydra/bodysplit") 
	light:SetModelScale(0.7)
...
function ENT:Touch( ent )
	if ent == self.target then
		...
		self:DoEndTarget()
		KillAndLeaveUsableRagdoll(ent, self:GetForward() * -4000, self:GetPos(), self.Entity, self.Entity)
	...
	if ent:GetClass() == "prop_ragdoll" && !harpoon && !self:IsConstrained() then
		...
		if self:GetPos():Distance(ent:GetPos()) < 30 then
			...
			timer.Simple(10, function() 
                ...
                ent:Remove()
				self:EmitSound("hydra/hydra_gulp.wav" ,80, 100)
            end)
		end
...
function ENT:PhysicsSimulate(phys, dtime)
	...
	phys:Wake()
	  ...
	  if self.act == "prestrike" then
		...
		self.ShadowParams.secondstoarrive = 0.4
		end
